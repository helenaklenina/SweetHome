...
def handle_dialog(req, res):
    ...
    if gate_serial != '':
        if gateproc.is_gate_connected(gate_serial):
            if req['session']['new']:
                # Это новая сессия.
                # ТУТ ВСЕ ОЧЕНЬ ВЫМЫШЛЕННОЕ! Естсественно никак не проверенное
                r = request.get('https://our_web_page/user/subs')
                ptint(r.text)
                if user_subs == 1:
                    res['response']['text'] = 'У вас закончилась подписка, ее необходимо продлить.'
                elif user_subs == 0:
